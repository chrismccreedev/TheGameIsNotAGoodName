...
public class TargetAcquisition : MonoBehaviour
{
    ...
    private void Line(Transform transform)
    {
        Debug.DrawLine(_camera.transform.position, transform.transform.position, Color.red, 10f);
    }
    ...
    public void NextHorizontalEnableCheck(int direction)
    {
        ...
        {
            float rotY = Rotate(obj);
            Debug.Log("1: " + startRot + "; 2: " + rotY + "; 3: " + (rotY - startRot) * direction);
            if((rotY - startRot) * direction > 0 && (rotY - startRot) * direction <= _rotateY)
            {
